...
GuiSlideshowScreensaverOptions::GuiSlideshowScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel(row, "다음 시간 후 이미지 변경(초)", sss_image_sec);
	...
	addWithLabel(row, "이미지 늘리기", sss_stretch);
	...
	addEditableTextComponent(row, "배경 오디오", sss_bg_audio_file, Settings::getInstance()->getString("SlideshowScreenSaverBackgroundAudioFile"));
	...
	addWithLabel(row, "사용자 이미지 사용", sss_custom_source);
	...
	addEditableTextComponent(row, "사용자 이미지 경로", sss_image_dir, Settings::getInstance()->getString("SlideshowScreenSaverImageDir"));
	...
	addWithLabel(row, "사용자 이미지 경로 순환", sss_recurse);
	...
	addEditableTextComponent(row, "사용자 이미지 필터", sss_image_filter, Settings::getInstance()->getString("SlideshowScreenSaverImageFilter"));
